...
kernel void mg_get_partial_sums_32_X_int(
   ...
   threadgroup  int threadgroup_partial_sums[32];
    ...
    // Number of the iterations must be the same for all the threads
    for (   size_t start_thread_position_in_grid_for_loop = c.num_threads_per_partial_sum * threadgroup_position_in_grid

          ; start_thread_position_in_grid_for_loop < c.num_threads_per_partial_sum * ( threadgroup_position_in_grid + 1 )

          ; start_thread_position_in_grid_for_loop += threads_per_threadgroup
    ) {

        size_t thread_position_in_grid_for_loop = start_thread_position_in_grid_for_loop + thread_position_in_threadgroup;

        local_sum_per_thread += ( (    thread_position_in_grid_for_loop < c.num_elements
                                    && thread_position_in_grid_for_loop < c.num_threads_per_partial_sum * ( threadgroup_position_in_grid + 1 )
                                  ) ? in[ thread_position_in_grid_for_loop ] : 0 );
    }
...
kernel void mg_scan_threadgroupwise_32_X_int(
    ...
    if  ( thread_position_in_grid < c.num_elements ) {
        inout[ thread_position_in_grid ] =  ( simdgroup_index_in_threadgroup > 0 )
...
kernel void mg_scan_final_32_X_int(
    ...
    // Number of the iterations must be the same for all the threads
    for (   uint start_thread_position_in_grid_for_loop = c.num_threads_per_partial_sum * threadgroup_position_in_grid

          ; start_thread_position_in_grid_for_loop < c.num_threads_per_partial_sum * ( threadgroup_position_in_grid + 1 )

          ; start_thread_position_in_grid_for_loop += threads_per_threadgroup
    ) {
        uint thread_position_in_grid_for_loop = thread_position_in_threadgroup + start_thread_position_in_grid_for_loop;
        ...
        thread int local_sum = (    thread_position_in_grid_for_loop < c.num_elements
                                 && thread_position_in_grid_for_loop < c.num_threads_per_partial_sum * ( threadgroup_position_in_grid + 1 )
                               ) ? in[ thread_position_in_grid_for_loop ] : 0 ;
        ...
        if (thread_position_in_grid_for_loop < c.num_elements) {
