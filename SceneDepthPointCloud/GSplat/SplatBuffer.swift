...
fileprivate let log =
    Logger(subsystem: Bundle.main.bundleIdentifier ?? "SplatBuffer",
           category: "SplatBuffer")

/// A resizable Metal buffer wrapper used by SplatRenderer
class SplatMetalBuffer<T> {
    ...
    /// Returns the index of the value
    @discardableResult
    func append(_ otherBuffer: SplatMetalBuffer<T>, fromIndex: Int) -> Int {
