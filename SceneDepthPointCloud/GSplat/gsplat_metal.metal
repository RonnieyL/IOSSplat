...
// Helper kernels for Radix Sort and Display

kernel void split_keys_kernel(
    const device long* keys [[buffer(0)]],
    device int* keys_high [[buffer(1)]],
    device int* keys_low [[buffer(2)]],
    uint id [[thread_position_in_grid]]
) {
    long key = keys[id];
    keys_high[id] = (int)(key >> 32);
    keys_low[id] = (int)(key & 0xFFFFFFFF);
}

kernel void combine_keys_kernel(
    const device int* keys_high [[buffer(0)]],
    const device int* keys_low [[buffer(1)]],
    device long* keys [[buffer(2)]],
    uint id [[thread_position_in_grid]]
) {
    long high = (long)keys_high[id];
    long low = (long)keys_low[id];
...
kernel void fill_long_kernel(
    device long* buffer [[buffer(0)]],
    constant long& value [[buffer(1)]],
    constant uint& start_idx [[buffer(2)]],
    constant uint& count [[buffer(3)]],
    uint id [[thread_position_in_grid]]
) {
    if (id >= count) return;
    buffer[start_idx + id] = value;
}
